...
*/
define fluent function isVerified(condition ConditionProblemsHealthConcerns):      
...
*/
define fluent function verified(conditions List<ConditionProblemsHealthConcerns>):           
...
Measure developers should note that not all EHRs provide both Encounter.reasonCode and Encounter.reasonReference. The below support both approaches to accommodate differences in EHR implementations. 
...
```
Alternatively, measure developers can use the encounterDiagnosis fluent function from the <strong style="color:red;">REVIEW NEEDED: verify library and function names.</strong>CQMCommon library to express the same logic as:
...
When identifying a history of a condition, elements such as clinicalStatus and whether the condition appears on the problem list, or as a health concern, or as an encounter diagnosis are often less relevant. For this reason, these attributes/elements are typically not referenced in basic historical queries. However, in US Quality Core, conditions are represented using two different profiles. As a result, separate retrievals are required to ensure complete capture:
...
```cql
CQL:
define "History Of Diabetes 2":
 ([USQualityCore.ConditionProblemsHealthConcerns: "Diabetes"] ConditionDM
    where ConditionDM.verificationStatus is not null implies ConditionDM.verificationStatus !~ FHIRCommon."refuted"
    ) 
     union (
      [ConditionEncounterDiagnosis: "Diabetes"] ConditionDM
        where ConditionDM.encounter.getEncounter().status = 'finished'  //added .encounter.
  )  
 
 define fluent function "getEncounter"(reference FHIR.Reference):            
 singleton from (
...
In US Quality Core, these choice types are constrained to forms that support actual computation of a prevalence period. The <strong style="color:red;">REVIEW NEEDED: verify library and function names.</strong>QICoreCommon library provides abatementInterval and prevalenceInterval functions to simplify access to this information. For example:
...
define "Active Diabetes Conditions Onset During The Measurement Period":         
  "Active Diabetes Conditions" DM
    where DM.prevalenceInterval() starts during day of "Measurement Period"
```
...
To express the concept of a condition present on admission (POA), measure developers should use the US Quality Core Claim profile. 
...
Alternatively, this logic can be expressed using the `claimDiagnosis` fluent function from the <strong style="color:red;">REVIEW NEEDED: verify library and function names.</strong>CQMCommon library:
...
```cql
CQL: <strong style="color:red;">REVIEW NEEDED: error noted in word doc.</strong>
define fluent function isDiagnosisPresentOnAdmission(encounter USQualityCore.Encounter, diagnosisValueSet ValueSet, poaValueSet ValueSet):
  exists (
...
#### Principal Diagnosis
A principal diagnosis represents the condition chiefly responsible for the patient’s admission. In US Quality Core, principal diagnosis information is available through the US Quality Core Claim profile <strong style="color:red;">REVIEW NEEDED: should we note this profile is out of scope</strong>using the diagnosis.type element or the sequence associated with the principal diagnosis.

Principal diagnoses can be retrieved either directly from the claim resource or through the <strong style="color:red;">REVIEW NEEDED: verify library and function names.</strong>CQMCommon claimDiagnosis fluent function.
...
define "Encounter With Principal Diagnosis of Asthma":
  [Encounter] E
    let
      ...
      claimItem: (claim.item I where I.encounter.references(E)),
      claimD: (claim.diagnosis D where D.sequence in claimItem.diagnosisSequence)
    where claimD.type.includesCode("Principal Diagnosis")
      and exists( claimD.diagnosis Diagnosis          //Need to account for the fact that Claim.diagnosis.diagnosis has cardinality of 0..*
        where Diagnosis in "Asthma"
        or Diagnosis.getCondition().code in "Asthma" 
      )
	  <strong style="color:red;">REVIEW NEEDED://comment.</strong>
```

Using the claimDiagnosis fluent function found in the <strong style="color:red;">REVIEW NEEDED: verify library and function names.</strong>CQM Common library:

```cql
CQL:
define "Encounter With Principal Diagnosis Of Asthma 2":
 [Encounter] ValidEncounter
   where exists (
    (ValidEncounter.claimDiagnosis()) D         
...
define fluent function hasPrincipalDiagnosisOf(encounter Encounter, valueSet ValueSet):
 encounter E
  let
    PD: singleton from ((E.claimDiagnosis()) Dx where Dx.type.includesCode("Principal Diagnosis")),
    CD: singleton from [FHIR.Condition] EncDx where PD.diagnosis.references(EncDx.id)
