...
1. Use the [US Quality Core Implementation Guide](http://fhir.org/guides/onc/us-quality-core) as the model
2. Make use of shared artifacts in the [Using CQL With FHIR](https://hl7.org/fhir/uv/cql) and [Common CQL Artifacts for FHIR (US-Based)](https://hl7.org/fhir/us/cql) implementation guides.
...
> NOTE: A CQL feature request has been submitted ([FHIR-58706](https://jira.hl7.org/browse/FHIR-58706)) to propose the ability to define an "alias" for a type name within a library, so that authors can say "In this library, the identifier "Encounter" always means the USQualityCore Encounter.
...
See the [Function Index](refactored_index.html#function-index) topic in the Refactored Index for a complete index of where refactored functions are now located.
