...
`topic()` is the accessor for the negated activity extent, defined in USQualityCoreCommon; it returns the topic whether it was recorded as a code or as a reference to a value set through the `notDoneValueSet` extension. See [Extent of negation](patterns.html#extent-of-negation).
